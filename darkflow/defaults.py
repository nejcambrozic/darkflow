        ...
        while i < len(args):
            ...
            if isinstance(self.get(argumentName), bool):
                if not (i + 1) >= len(args) and (
                        args[i + 1].lower() != 'false' and args[i + 1].lower() != 'true') and not \
                        args[i + 1].startswith('--'):
                    print(
                        'ERROR - Expected boolean value (or no value) following argument: ' + args[
                            i])
                    print('Try running flow --help')
                    exit()
            ...
            elif args[i].startswith('--') and not (i + 1) >= len(args) and not args[
