...
def find_accuracy(path, jpath):
    ...
    for b, file in enumerate(glob.glob("*.xml")):
        ...
        # modify names of detections because of working with different output models
        names_det = ['obstacles' for _ in names_det]
        ...
        print("undetected:", len(undetected), "    True positives:", len(True_Positive),
              "   False positives:", len(False_Positive))

        if len(True_Positive) + len(False_Positive) + len(undetected) > 0:
            tps += len(True_Positive)
            fps += len(False_Positive)
            fns += len(undetected)

            accuracy = float(2 * len(True_Positive)) / (
                    2 * len(True_Positive) + len(False_Positive) + len(undetected))
...
def main():
    annotation_path = "/Users/nejc/github/msc/datasets/modd1_1c_dataset/Annotations/"
    json_path = "/Users/nejc/github/mAP/predicted/backup/"
